...
void master_vma_open(struct vm_area_struct *vma){
	printk(KERN_NOTICE "master_VMA open");
}
void master_vma_close(struct vm_area_struct *vma){
	printk(KERN_NOTICE "master VMA close.\n");
}
static struct vm_operations_struct master_remap_vm_ops={
...
static long master_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	switch(ioctl_num){
		...
		case master_IOCTL_MMAP:
			//printk("in mmap : \n");
			//printk("%s\nSize = %lld", file->private_data, ioctl_param);
			ksend(sockfd_cli, file->private_data,  ioctl_param, 0);
			//printk("[page] : %lX\n" , (u64)virt_to_phys(file->private_data));
			//printk("size = %d\n" , sizeof(virt_to_phys(file->private_data)));	
			break;
...
static int master_map(struct file *file, struct vm_area_struct *vma){
    ...
    //printk("[page] : %lX\n" , (u64)virt_to_phys(file->private_data));
    //printk("[page] : %16lx\n" , vma->vm_start);
	//ksend(sockfd_cli, file->private_data, size, 0);
	return 0;	
}
